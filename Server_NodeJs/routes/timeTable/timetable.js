...
router.post("/createTable/",function(request,response){
 ...
 router.get("/detailView",function(req,res){
    if (req.session.loggedin) {
        ...
        var data;
        db.query('SELECT (select COUNT(*)   FROM rooms  ) AS totalRooms,( select COUNT(*)    FROM period) AS totalPeriods ,(select COUNT(*)    FROM subjects) AS totalSubjects,(select COUNT(*)    FROM faculty) AS totalFaculty,(select COUNT(*)    FROM students) AS totalStudent',function(error,result){
            if(error){
                throw error;
            }
                 data=result;
