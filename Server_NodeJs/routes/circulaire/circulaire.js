...
router.get("/circulaireView",function(req,res){
        ...
        db.query('select * from circulaire',function(error,result){
            if(error){
                throw error;
                    ...
                    data=result;
                res.render("circulaireView",{json:JSON.parse(JSON.stringify(data))});
        });
        }else{
            res.render("index");
        }
    ...
    router.post("/uploadCirculaire",function(request,response){
        ...
        if (request.session.loggedin)
        {
            ...
            pdfFile.mv(path.join(__dirname,'../../uploads/', pdfFile.name), function (err) {
                ...
                } else {  db.query("INSERT INTO circulaire (name, posted_on, path) VALUES (?, NOW(),?)",[request.body.name, realpath], function(err, result) {
                ...
                response.redirect('circulaireView');
            }
        });
    } else {
